...
file_handler = logging.FileHandler('by_all_capacity_reservations.log')
...
# checking if SNS topic with topic_name 'ODCRAlarmTopic' exists, return topic_arn else none.
...
# creates an alarm for InstanceUtilization CW metric with InstanceTyp All_Capactity dimension and if SNS topic not created in a region, it will create one.
def createCWAlarm(topic_name,Dimension,MetricName,RegionName,EmailAddress,Protocol,ComparisonOperator,Threshold):
    ...
    response = cw.put_metric_alarm(
        AlarmName=f'ODCRAlarm-{MetricName}-{Dimension}',
        AlarmActions=[
        topic_arn,
        ],
...
def main():
    ...
    parser.add_argument(
        '--Dimension', 
        type=str, 
        default='AllCapacityReservations', 
        help="The dimension for the CloudWatch alarm. Default is 'AllCapacityReservations'."
    )
