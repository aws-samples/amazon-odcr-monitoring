...
file_handler = logging.FileHandler('by_availabilityZone.log')
...
'''
#list SNS Topics per region
def listTopic(RegionName):
    try:
        sns = boto3.client('sns', region_name=RegionName)
        # Initialize the paginator for the list_topics method
        ...
        for page in page_iterator:
            ...
            for topic in topics:
                ...
                if arn.split(':')[-1] == topic_name:
                    return arn
                TopicArn.append(topic['TopicArn'])
        logger.info(f"TopicArn: {TopicArn}")            
        return TopicArn  
...
# creates an alarm for InstanceUtilization CW metric with by_availabilityZone dimension and if SNS topic not created in a region, it will create one.
def createCWAlarm(topic_name,Dimension,MetricName,RegionName,EmailAddress,Protocol,ComparisonOperator,Threshold,AvailabilityZone):
    ...
    response = cw.put_metric_alarm(
        AlarmName=f'ODCRAlarm-{MetricName}-{Dimension}-{AvailabilityZone}',
        AlarmActions=[
        topic_arn,
        ],
...
def main():
    ...
    parser.add_argument(
        ...
        help="The dimension for the CloudWatch alarm. Default is 'AvailabilityZone'."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,  # This makes the argument required
        help="The availability zone for the CloudWatch alarm. Example: 'us-east-1a'."
    )
    parser.add_argument(
        '--TopicName',
        type=str,
