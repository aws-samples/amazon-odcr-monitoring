...
file_handler = logging.FileHandler('by_instanceType.log')
...
# creates an alarm for InstanceUtilization CW metric with by_instanceType dimension and if SNS topic not created in a region, it will create one.
def createCWAlarm(topic_name,Dimension,MetricName,RegionName,EmailAddress,Protocol,ComparisonOperator,Threshold,InstanceType):
    ...
    response = cw.put_metric_alarm(
        AlarmName=f'ODCRAlarm-{MetricName}-{Dimension}-{InstanceType}',
        AlarmActions=[
        topic_arn,
        ],
...
def main():
    ...
    parser.add_argument(
        ...
        help="The dimension for the CloudWatch alarm. Default is 'InstanceType'."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,  # This makes the argument required
        help="The InstanceType for the CloudWatch alarm. Example: 't2.micro'." 
    )
    parser.add_argument(
        '--TopicName',
        type=str,
